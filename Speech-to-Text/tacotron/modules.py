...
def normalize_bn(inputs, decay=.99,is_training=True,activation_fn=None,scope="normalize_bn"):
...
def attention_decoder(inputs,memory,units=None,scope="attention_decoder",reuse=None):
    with tf.variable_scope(scope, reuse=reuse):
        ...
        cell_with_attention = tf.contrib.seq2seq.AttentionWrapper(decoder_cell, attention_mechanism, units)
...
def prenet(inputs, is_training=True, scope="prenet", reuse=None):
    with tf.variable_scope(scope, reuse=reuse):
        ...
        outputs = tf.layers.dense(outputs, units=embed_size//2, activation=tf.nn.relu, name="dense2")
