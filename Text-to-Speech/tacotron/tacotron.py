    ...
    def __init__(self, is_training=True, reuse=None):
        ...
        with tf.variable_scope('encoder', reuse=reuse):
            ...
            for i in range(num_highwaynet_blocks):
                enc = highwaynet(enc, num_units=embed_size//2, scope='encoder-highwaynet-{}'.format(i))
        ...
        with tf.variable_scope('decoder-1', reuse=reuse):
            ...
            with tf.variable_scope('attention-decoder-1', reuse=reuse):
                ...
                outputs_attention, state_attention = tf.nn.dynamic_rnn(cell_with_attention, 
                                                                       prenet_out_decoder1, 
                                                                       dtype=tf.float32)
        ...
        with tf.variable_scope('decoder-2', reuse=reuse):
            ...
            for i in range(4):
                dec = highwaynet(dec, num_units=embed_size//2, scope='decoder-highwaynet-{}'.format(i))
        ...
        self.lr = learning_rate * 4000 ** 0.5 * tf.minimum(step * 4000 ** -1.5, step ** -0.5)
